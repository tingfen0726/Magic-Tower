...
void Map::LoadLevel(const std::vector<std::vector<int>>& levelData) {
    ...
    for (int y = 0; y < m_LevelData.size(); y++) {
        for (int x = 0; x < m_LevelData[y].size(); x++) {
            ...
            if (blockID >= Config::ID::NPC_BEGIN && blockID <= Config::ID::NPC_END) {
                m_Blocks.push_back(LoadNPCs(blockID, x, y));
            }
            else {
                switch (blockID) {
...
std::shared_ptr<NPC> Map::LoadNPCs(int blockID, int x, int y) {
    switch (blockID) {
        case Config::ID::FAIRY_0:
            return std::make_shared<FairyNPC>(
                 std::vector<std::string>{
        ...
        case Config::ID::THIEF_4:
            return std::make_shared<ThiefNPC>(
                 std::vector<std::string>{
        ...
        case Config::ID::ELDER_2:
            return std::make_shared<DonorNPC>(
                 std::vector<std::string>{
        ...
        case Config::ID::SHOPKEEPER_15:
            return std::make_shared<TraderNPC>(
                 std::vector<std::string>{
                     RESOURCE_DIR "/Image/NPC/shopkeeper.bmp",
                     RESOURCE_DIR "/Image/NPC/shopkeeper2.bmp",
                     RESOURCE_DIR "/Image/NPC/road.bmp"
        ...
        case Config::ID::ELDER_16:
            return std::make_shared<ElderNPC>(
                 std::vector<std::string>{
                     RESOURCE_DIR "/Image/NPC/elder.bmp",
                     RESOURCE_DIR "/Image/NPC/elder2.bmp",
                     RESOURCE_DIR "/Image/NPC/road.bmp"
        ...
        case Config::ID::PRINCESS:
            return std::make_shared<princessNPC>(
                 std::vector<std::string>{
        ...
        case Config::ID::VAMPIRE_19:
            return std::make_shared<vampireNPC>(
                 std::vector<std::string>{
                     RESOURCE_DIR "/Image/NPC/road.bmp"
                 },
        ...
        case Config::ID::SYSTEM_NPC:
            return std::make_shared<systemNPC>(
                 std::vector<std::string>{
